...
struct ComplStat {
    ...
    std::string ecnsupport;
    std::ofstream *outfile;

    ComplStat(uint32_t nr_bins_norm, uint32_t rtt, uint32_t rate, std::string foldername, std::string ecn) 
    {
        m_rtt = rtt;
        m_rate = rate;
        ...
        filename << m_foldername << "/bins_" << ecn;
...
{
    printf("Usage: %s <folder> <avgrate_ecn> <avgrate_nonecn> <rtt> <link>\n", argv[0]);
    exit(1);
}
...
void processFile(std::string folder, std::string fn_in, std::string fn_out, std::string fn_bins, uint32_t rtt, double link_bytes_ps, double avg_rate_pf, std::string ecn)
{    
    uint32_t nr_bins_norm = size_bins.size();
    ComplStat *complstat = new ComplStat(nr_bins_norm, rtt, link_bytes_ps, folder, ecn);
...
int main(int argc, char **argv)
{
    ...
    double avgrate_ecn = atof(argv[2]);
    double avgrate_nonecn = atof(argv[3]);
    ...
    processFile(folder, "/completion_time_ecn_w_hs", "/compl_ecn_w_hs", "/compl_bins_ecn_hs", rtt, link_bytes_ps, avgrate_ecn, "ecn");
    processFile(folder, "/completion_time_nonecn_w_hs", "/compl_nonecn_w_hs", "/compl_bins_nonecn_hs", rtt, link_bytes_ps, avgrate_nonecn, "nonecn");
